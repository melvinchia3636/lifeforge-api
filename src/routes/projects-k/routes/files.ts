...
import { IProjectsKEntry } from '../../../interfaces/projects_k_interfaces.js'
...
router.get(
    ...
    asyncWrapper(async (req: Request, res: Response) => {
        const { pb } = req
        const project: IProjectsKEntry = await pb
        ...
        files.forEach(file => {
            ...
            fs.copyFileSync(
                location,
                `/media/${process.env.DATABASE_OWNER}/uploads/${file.split('.')[0].split('_').slice(0, -1).join('_')}.${file.split('.').pop()}`
            )
        })
...
router.put(
    ...
    asyncWrapper(async (req: Request, res: Response) => {
        ...
        if (!['image/png', 'image/jpeg'].includes(type ? type : '')) {
            return res.status(401).json({
                state: 'error',
        ...
        await pb.collection('projects_k_entries').update(req.params.projectId, {
            thumbnail: new File([buffer], file, { type: type ? type : '' }),
