...
router.get('/list/:difficulty', asyncWrapper(async (req, res) => {
    const { pb } = req;
    ...
    const achievements = await pb.collection('achievements_entry').getFullList({
...
router.patch('/update/:id', asyncWrapper(async (req, res) => {
    ...
    const achievement = await pb.collection('achievements_entry').update(id, {
        difficulty,
        title,
        thoughts,
    });
